...
class SitioList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if word is not None:
            ...
            original_word_regxp = create_accents_regular_expression(original_word)

            word = inflector.singularize(word)
              
            word = remove_accents(word.encode('utf-8'))
            word_regxp = create_accents_regular_expression(word)
        ...
        raw_query = raw_query.format(original_word=original_word,word=word,municipio_query=municipio_query, original_word_regxp=original_word_regxp, word_regxp=word_regxp)
